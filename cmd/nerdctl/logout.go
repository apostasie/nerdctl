...
func logoutAction(cmd *cobra.Command, args []string) error {
	...
	err, errGroup := credentialsStore.Logout(logout)
	if err != nil {
		log.L.WithError(err).Error("Failed to erase credentials for: %q", logout)
	}
	if errGroup != nil {
...
func logoutShellComplete(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	candidates, err := credentialsStore.LogoutShellCompletion()
	if err != nil {
		return nil, cobra.ShellCompDirectiveError
	}
